...
import { construirCamposPorRol, puedeEditarReporte, puedeEliminarUsuario, esReporteNuevo, normalizarTexto, coincideBusqueda, compararReportes } from './reportRules';
...
describe('compararReportes', () => {
  const reportes = [
    { consecutivo: 3, createdAt: '2026-01-03' },
    { consecutivo: 1, createdAt: '2026-01-01' },
    { consecutivo: 2, createdAt: '2026-01-02' },
  ];

  it('consecutivo_asc ordena del 1 al último', () => {
    const ordenados = [...reportes].sort((a, b) => compararReportes(a, b, 'consecutivo_asc'));
    expect(ordenados.map((r) => r.consecutivo)).toEqual([1, 2, 3]);
  });

  it('consecutivo_desc ordena del último al 1', () => {
    const ordenados = [...reportes].sort((a, b) => compararReportes(a, b, 'consecutivo_desc'));
    expect(ordenados.map((r) => r.consecutivo)).toEqual([3, 2, 1]);
  });

  it('sin orden explícito, cae al comportamiento por defecto (más reciente primero)', () => {
    const ordenados = [...reportes].sort((a, b) => compararReportes(a, b, undefined));
    expect(ordenados.map((r) => r.consecutivo)).toEqual([3, 2, 1]);
  });
});
